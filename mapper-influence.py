...
# Jared Culp (jjc4fb)
# CS 4501 - HW5 prelim
# This one factors in retweets/favorites/number of followers
...
if (counts_pos - counts_neg) == 0:
    pass
elif (is_dragas > 0) and (is_sully > 0): # do nothing if both mentioned
    pass
elif (is_dragas + is_sully) == 0: # do nothing if neither are mentioned
    pass
elif is_dragas > is_sully:
    print '%s\td\t%d' % (date, (counts_pos - counts_neg) * factor)
else:
    print '%s\ts\t%d' % (date, (counts_pos - counts_neg) * factor)
